...
func Test_GuiSetSystemtray(t *testing.T) {
	ec := templater.CreateEventChannel()
	systemTray := new(SystemtrayMock)
	var inter interface{} = systemTray

	gui := CreateGui(ec)
	gui.SetSystemtray(systemTray)
...
func Test_GuiSetApi(t *testing.T) {
	ec := templater.CreateEventChannel()
	api := new(ApiMock)
